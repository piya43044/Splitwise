...
export class DashboardComponent implements OnInit {
  ...
  getBorrowedAmountList(): void {
    ...
    this.dashboardService.getBorrowedAmountList().subscribe(
      ...
      for (let item of data) {
        this.dashboardService.getUserNameById(item.whomeToGive).subscribe(
          ...
          (error) => {
            this.toastrService.success('No Borrowed amount of ' + this.userName, '', {
              timeOut: 2000,
            });
          })
      }
    ...
    (error)=>{
      this.toastrService.success('No Borrowed amount details of ' + this.userName, '', {
        timeOut: 2000,
      });
    })
  ...
  getDebtAmountList(): void {
    ...
    this.dashboardService.getDebtAmountList().subscribe(
      ...
      for (let item of data) {
        this.dashboardService.getUserNameById(item.owesFromYou).subscribe(
          ...
          (error) => {
            this.toastrService.success('No debt amount of ' + this.userName, '', {
              timeOut: 2000,
            });
          })
      }
    ...
    (error)=>{
      this.toastrService.success('No debt amount details of ' + this.userName, '', {
        timeOut: 2000,
      });
    })
